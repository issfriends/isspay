...
func (s *OrderSvcSuite) SetupTest() {
	err := s.TruncateTables("orders", "ordered_products", "products", "wallets", "accounts")
	s.Require().NoError(err)
}

func (s *OrderSvcSuite) TestCreateOrder() {
	products := factory.Product.Price(100).MustInsertN(20).([]*model.Product)
	productsMap := make(map[uint64]*model.Product)
	...
	for _, tc := range tcs {
		...
		s.Run(tc.name, func() {
			w, ops := tc.setup()
			beforeQuantity := make(map[uint64]uint64)
			productIDs := make([]uint64, 0, len(ops))
			opMap := make(map[uint64]*model.OrderedProduct)
			cost := decimal.Zero
			for _, op := range ops {
			...
			o := model.NewOrder(w.ID, ops...)
			balance, err := s.Svc.Order.CreateOrder(s.Ctx, o)
			if tc.happy {
				s.Require().NoError(err)
				s.Assert().True(w.Amount.Sub(cost).Equal(balance), "wallet amount should be correct")
			} else {
				s.Require().Error(err)
			}
			...
			_, err = s.Database.ListProducts(s.Ctx, listProducts)
			s.Require().NoError(err)
...
func (s *OrderSvcSuite) TestCancelOrder() {
	...
	for _, tc := range tcs {
		s.Run(tc.name, func() {
			w, o := tc.setup()
			balance, _, err := s.Svc.Order.CancelOrder(s.Ctx, w.ID, o.UID)
			...
			quantity := make(map[uint64]uint64)
			productIDs := make([]uint64, 0, 1)
